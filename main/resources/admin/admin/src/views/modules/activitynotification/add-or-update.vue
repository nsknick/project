<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="通知时间" prop="notificationTime">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.notificationTime" label="通知时间" prop="notificationTime">
              <el-input v-model="ruleForm.notificationTime"
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="通知内容" prop="notificationContent">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.notificationContent" label="通知内容" prop="notificationContent">
                    <span>{{ruleForm.notificationContent}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
,
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `activitynotification/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `activitynotification/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.activitynotificationCrossAddOrUpdateFlag = false;
						 this.parent.search();
